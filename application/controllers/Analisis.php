...
class Analisis extends CI_Controller {
	...
	public function lista_analisis_paciente()
	{
		$allInputs = json_decode(trim(file_get_contents('php://input')),true);
		//$paramPaginate = $allInputs['paginate'];

		$lista = $this->model_analisis->m_cargar_analisis_paciente($allInputs);
		//$totalRows = $this->model_analisis->m_count_analisis($allInputs);
		$arrListado = array();
		foreach ($lista as $row) {
			array_push($arrListado,
				array(
	...
	public function lista_analisis_paciente_idanalisis()
	{
		$allInputs = json_decode(trim(file_get_contents('php://input')),true);
		//$paramPaginate = $allInputs['paginate'];

		$lista = $this->model_analisis->m_cargar_analisis_paciente_idanalisis($allInputs);
		//$totalRows = $this->model_analisis->m_count_analisis($allInputs);
		$arrListado = array();
		foreach ($lista as $row) {
			array_push($arrListado,
